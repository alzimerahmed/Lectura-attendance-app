...
class LumeraRepository(
    ...
    suspend fun updateTimetableEntry(entry: TimetableEntryEntity) {
        ...
        if (oldEntry != null) {
            ...
            for (session in linkedSessions) {
                ...
                val isMarked = sessionUnits.any { it.status != com.alzimerahmed.lumera.domain.model.AttendanceStatus.UNMARKED.name }
    ...
    suspend fun updateTimetableEntryFromDate(
        ...
        if (oldEntry != null) {
            ...
            val hasPastMarkedSessions = pastSessions.any { session ->
                ...
                attendanceDao.getUnitsForSession(session.id).any { it.status != com.alzimerahmed.lumera.domain.model.AttendanceStatus.UNMARKED.name }
            }
    ...
    suspend fun importParsedTimetable(
        items: List<com.alzimerahmed.lumera.domain.model.ParsedTimetableItem>,
