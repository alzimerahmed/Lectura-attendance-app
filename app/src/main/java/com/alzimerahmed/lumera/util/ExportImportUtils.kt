...
import com.alzimerahmed.lumera.data.local.entity.*
...
data class TimetableSharePackage(
    val format: String = "Lumera_TIMETABLE_V1",
    val exportedAt: Long = System.currentTimeMillis(),
    val subjects: List<SubjectEntity> = emptyList(),
    val timetableEntries: List<TimetableEntryEntity> = emptyList()
)
...
object ExportImportUtils {
    ...
    suspend fun exportTimetableToJson(repository: LumeraRepository): String {
    ...
    suspend fun exportToJson(repository: LumeraRepository): String {
        val backup = LumeraBackup(
            version = 1,
            exportedAt = System.currentTimeMillis(),
    ...
    suspend fun exportToCsv(repository: LumeraRepository): String {
    ...
    suspend fun exportDataToJson(context: Context, repository: LumeraRepository, uri: Uri): Boolean {
        val json = exportToJson(repository)
        return writeTextToUri(context, uri, json)
    }

    suspend fun importDataFromJson(context: Context, repository: LumeraRepository, uri: Uri): Pair<Boolean, String> {
    ...
    suspend fun exportAttendanceCsv(context: Context, repository: LumeraRepository, uri: Uri): Boolean {
